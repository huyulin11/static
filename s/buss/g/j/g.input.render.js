...
export var getInputHtml = (item, _defaultValue, _serial) => {
    ...
    if ("select" == item.type) {
        ...
        itemDesc += `<option value="">----选择【${name}】----</option>`;
        for (let dic of dics) {
            let selectFlag = (data && dic.key == data[item.key]) ? "selected" : "";
            itemDesc += `<option ${selectFlag} value="${dic.key}">${dic.value}</option>`;
        }
        itemDesc = `<select id="${id}" name="${id}" data-notnull='${item.notnull}'>${itemDesc}</select>`;
    } else if ("jsSelect" == item.type) {
        itemDesc += `
        <select class="form-control ${item.type}" data-patten="${item.patten}" name="${id}"
        ...
        data-showcol='${item.showcol}' placeholder="输入:${name}" 
        data-notnull='${item.notnull}' autocomplete="off">`;
    } else {
        itemDesc = `
    <input type="text" id="${id}" name="${id}" 
        class="form-control" value="${_defaultValue ? _defaultValue : ""}"
        placeholder="输入:${name}" data-notnull='${item.notnull}' autocomplete="off">`;
    }
    return itemDesc;
}
