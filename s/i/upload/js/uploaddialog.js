 ...
 * @return 
 */
function Dialog(content, options,title,tag)
{
    var defaults = { // 默认值。 
        title:'我的图片',       // 标题文本，若不想显示title请通过CSS设置其display为none 
    ...
    var barHtml = !options.showTitle ? '' :
        '<div class="bar" id="div_bar" style="height:30px;"><span class="title">' + options.title + '</span><span class="close_div"> 关闭</span></div>';
    var dialog = $('<div id="' + options.id + '" class="dialog">'+barHtml+'<div class="content"></div><button class="btn_close">关闭</button></div>').hide();
    $('body').append(dialog);
    ...
    var init = function()
    {
        ...
        /* 绑定一些相关事件。 */       
        //dialog.find('.close_div').bind('click',this.close);
		//dialog.find('.btn_close').bind('click',this.close);
