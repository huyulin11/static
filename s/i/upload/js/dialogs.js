 ...
 * @return 
 */
function Dialogs(content, options,title,tag)
{
    var defaults = { // 默认值。 
    ...
    var barHtml = !options.showTitle ? '' :
        '<div class="bar" style="height:30px;"><span class="title">' + options.title + '</span><span class="closes_div"> 结束</span></div>';
    var dialog = $('<div id="' + options.id + '" class="dialog">'+barHtml+'<div class="content"></div><button class="btn_closes">结束</button></div>').hide();
    $('body').append(dialog);
