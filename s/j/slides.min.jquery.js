...
(function(A) {
	A.fn.slides = function(B) {
		...
		return this.each(function() {
			A("." + B.container, A(this)).children("a").wrapAll('<div class="slides_control"/>');
			var V = A(this),
				...
				if (!H && U) {
					...
					if (b === "fade") {
						if (B.crossfade) {
							...
							J.children(":eq(" + N + ")", V).fadeOut(B.fadeSpeed, B.fadeEasing,
							function() {
								if (B.autoHeight) {
									J.animate({
										height: J.children(":eq(" + O + ")", V).outerHeight()
...
//				position: "relative"
			...
			if (B.preload && J.find("img:eq(" + D + ")").length) {
				...
				J.find("img:eq(" + D + ")").attr("src", Y).load(function() {
					J.find(T + ":eq(" + D + ")").fadeIn(B.fadeSpeed, B.fadeEasing,
					function() {
						A(this).css({
							zIndex: 5
						});
				...
				J.children(":eq(" + D + ")").fadeIn(B.fadeSpeed, B.fadeEasing,
				function() {
					U = true;
					B.slidesLoaded();
				});
			...
			if (B.generateNextPrev) {
				A("." + B.container, V).after('<a  class="' + B.prev + '"></a>');
				A("." + B.prev, V).after('<a  class="' + B.next + '"></a>');
			}
