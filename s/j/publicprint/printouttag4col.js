...
function checkstartposition() {
	...
	if (startposition.length == 0) {
		$("#span_startposition").html("请输入起始位置，如:D1");
		$("#txt_startposition").css("border-color", "#C63302");
		return false;
	} else if (startposition.length == 1) {
		$("#span_startposition").html("输入错误，如:D1");
		$("#txt_startposition").css("border-color", "#C63302");
		return false;
	} else if (starthead == "A" || starthead == "B" || starthead == "C"
			|| starthead == "D") {
		if (starttail <= 12 * nowpage && starttail > 0) {
			$("#hidden_startposition").val(returnnum(starthead, starttail));
			return true;
		} else {
			$("#span_startposition").html("当前页面不能为空，如:D1");
			$("#txt_startposition").css("border-color", "#C63302");
			return false;
		}
	} else {
