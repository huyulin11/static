...
function checkstartposition() {
	...
	if (startposition.length == 0) {
		$("#span_startposition").html("请输入起始位置，如:A1");
		$("#txt_startposition").css("border-color", "#C63302");
		return false;
	} else if (startposition.length == 1) {
		$("#span_startposition").html("输入错误，如:A1");
		$("#txt_startposition").css("border-color", "#C63302");
		return false;
	} else if (starthead == "A" || starthead == "A") {
		if (starttail <= 3 * nowpage && starttail > 0) {
			$("#hidden_startposition").val(returnnum(starthead, starttail));
			return true;
		} else {
			$("#span_startposition").html("当前页面不能为空，如:A1");
			$("#txt_startposition").css("border-color", "#C63302");
			return false;
		}
	} else {
...
function hideWhenPrint() {
	$(".hide_when_print").hide();
}

function showWhenNotPrint() {
