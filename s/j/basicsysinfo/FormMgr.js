function FormMgr(){
		...
		this.columnsDetail = new ColumnsDetail()
				.createColArrays(this.tableInfo);
		this.idInEditForm ; 
		this.form = document.getElementById('perInfoModifyfrm');
		this.formtable = $('form#perInfoModifyfrm table tbody')[0];
		...
		'rebuildForm' : function rebuildForm(operateType){
			...
			if (trArray){
				...
				for (var i = 0 ;i < trArray.length ; i++){
					var tr = trArray[i] ;
					if (tr){
						var insertPosition = document.getElementById(
								this.reservedTrArray[this.reservedTrArray.length -1]);
